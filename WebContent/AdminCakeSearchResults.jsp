            ...
            <div class="panel panel-default table-responsive">
    		 ...
    		 <%
                ...
                if (request.getAttribute("CakeList") != null) {
                    ArrayList cakelist = (ArrayList) request.getAttribute("CakeList");
                    Iterator itr = cakelist.iterator();
                    while (itr.hasNext()) {
                        count++;
                        ArrayList CakeList = (ArrayList) itr.next();
            %>              
            	<tr>
            		<td><%=CakeList.get(0)%></td>
                    <td><%=CakeList.get(1)%></td>
                    <td><%=CakeList.get(2)%></td>
                    <td><%=CakeList.get(3)%></td>
                    <td><%=CakeList.get(4)%></td>
                    <td><%=CakeList.get(5)%></td>
        		</tr>
          <%
                    }
            ...
            %>          
          	...
          	<div class="text-center">
         	 <a class="btn btn-primary " href="AdminViewCakes.jsp">Back</a>   
          	</div>
        </div>
      </div>
	    </div>
